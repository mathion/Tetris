...
public class Square extends AbstractPiece{
  ...
  //rotate toddy code
  public boolean canRotate(Commands direction) {
	  boolean ableToRotate = true;
	  return ableToRotate;
  }
  //rotate toddy code
  public void rotate(Commands direction)
  {
	  
	  if(canRotate(direction))
	  {
		/* for(int i = 0; i  < AbstractPiece.PIECE_COUNT;i ++) {
			 switch (command) {
		        case DOWN:
		          row = row + 1;
		          break;
